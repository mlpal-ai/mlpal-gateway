...
| Curated catalog | `GET /v1/catalog`, `GET /v1/models` (+ `/aliases`), `GET /v1/messages/models` | Routable model attributes / tiers / effort levels, scoped to the models this key's `model_policy` allows: denied models are absent from every listing incl. tier ladders and meta-alias routings; `/v1/models/{tag}` is 403 for a denied tag; `/v1/models` reports `denied_by_policy` and the key's own policy. |
